...
// ── FP1 region protocol (attr 0x0150 write / 0x0151 event) ──
//
// Protocol verified against Koenkk/zigbee-herdsman-converters src/lib/lumi.ts
// (writeAqaraFp1RegionUpsert/Delete + lumi_presence_region_events, June 2026).
// Region grid is a fixed 7 rows (Y: 1-7) x 4 columns (X: 1-4), 10 regions max.
//
// Kept here as a verified reference implementation, but deliberately NOT
// wired into any device/UI code: the write side has no readback, and the
// official Aqara app's real-time zone-editor overlay (showing where it's
// currently detecting something while you draw) comes from data outside
// this documented protocol — without that feedback loop, a Homey UI would
// have you drawing zones blind. See README.txt.

const REGION_CMD = { CREATE: 1, DELETE: 3 }; // cmd 2 ("modify") is documented
// upstream as broken — it stops the region from reporting; always use CREATE
// for both initial creation and edits (it force-replaces the existing one).
const REGION_SUFFIX = { UPSERT: 0xFF, DELETE: 0x00 };
const REGION_EVENT_TYPES = { 1: 'enter', 2: 'leave', 4: 'occupied', 8: 'unoccupied' };

function encodeXCellsDefinition(xCells) {
  if (!xCells || !xCells.length) return 0;
  return xCells.reduce((acc, x) => acc + (2 ** (x - 1)), 0);
}

/**
 * Build the 7-byte payload to create (or force-replace) a region.
 * @param {number} regionId - 1-10
 * @param {Array<{x:number,y:number}>} zones - x:1-4, y:1-7
 * @returns {Buffer}
 */
AqaraLumiCluster.encodeRegionUpsert = function(regionId, zones) {
  const byRow = {};
  ...
  buf[0] = REGION_CMD.CREATE;
  buf[1] = regionId;
  buf[2] = encodeXCellsDefinition(byRow[1]) | (encodeXCellsDefinition(byRow[2]) << 4);
  buf[3] = encodeXCellsDefinition(byRow[3]) | (encodeXCellsDefinition(byRow[4]) << 4);
  buf[4] = encodeXCellsDefinition(byRow[5]) | (encodeXCellsDefinition(byRow[6]) << 4);
  buf[5] = encodeXCellsDefinition(byRow[7]);
  buf[6] = REGION_SUFFIX.UPSERT;
  return buf;
};

/**
 * Build the 7-byte payload to delete a region.
 * @param {number} regionId - 1-10
 * @returns {Buffer}
 */
AqaraLumiCluster.encodeRegionDelete = function(regionId) {
  const buf = Buffer.alloc(7);
  buf[0] = REGION_CMD.DELETE;
  buf[1] = regionId;
  buf[6] = REGION_SUFFIX.DELETE;
  return buf;
};

/**
 * Decode a region occupancy event (attr 0x0151): 2 bytes, [regionId, eventTypeCode].
 * @param {Buffer} buf
 * @returns {{regionId: number, event: string}|null}
 */
AqaraLumiCluster.decodeRegionEvent = function(buf) {
  if (!Buffer.isBuffer(buf) || buf.length < 2) return null;
  const event = REGION_EVENT_TYPES[buf[1]];
  if (!event) return null;
  return { regionId: buf[0], event };
};
